...
class PokerHand
  ...
  def four_of_a_kind?
    uniq_ranks.size == 2 unless full_house?
  end
  ...
  def three_of_a_kind?
    uniq_ranks.any? { |rank| card_ranks.count(rank) == 3 }
  end

  def two_pair?
    uniq_ranks.count { |rank| card_ranks.count(rank) == 2 } == 2
  end

  def pair?
    uniq_ranks.any? { |rank| card_ranks.count(rank) == 2 }
  end
